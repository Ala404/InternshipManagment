<template>
    <div class="box-border bg-white rounded-lg p-2 hover:bg-gray-300 cursor-pointer mb-2">
      <div class="flex items-start max-md:flex-col max-md:gap-8">
        <div class="flex justify-between items-center text-left max-md:text-center w-full max-md:flex-col max-md:gap-8">
          <img class="w-24 object-cover self-center rounded-lg mr-4" :src="image" alt="Company Logo" />
          <span class="flex justify-between items-center w-1/2 max-md:flex-col max-md:gap-8">
            <h2 class="text-xl font-bold underline text-primary pb-2">{{ title }}</h2>
            <!-- application status icon -->
            <span class="flex gap-1 mr-28">
              <i v-if="status == 'accepted'" class="fas fa-check-circle text-green-500 text-3xl"></i>
              <i v-if="status == 'pending'" class="fas fa-clock text-orange-500 text-3xl mr-12"></i>
              <i v-if="status == 'refused'" class="fas fa-times-circle text-red-500 text-3xl ml-56"></i>
            </span>
          </span>

          <div class="flex gap-3">
            <router-link :to="`/applications/${id}`">
              <i v-if="status == 'refused' || status == 'pending'" class="fas fa-edit p-3 my-2 mx-4 bg-white border-2 rounded-full text-orange-500 border-orange-500 hover:bg-orange-500 hover:text-white transition-all"></i>
            </router-link>
