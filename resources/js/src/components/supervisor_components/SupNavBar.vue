...
<template >
  ...
  >
    ...
    <div class="">
      ...
      >
      <Menu ref="menu" id="overlay_menu" :model="items" :popup="true" class="w-56 h-56 overflow-y-scroll"/>
    </div>
...
export default {
  ...
  data() {
    return {
      user: {
        ...
        avatarUrl: "../../../public/img/supervisor.jpg",
      },
      showDropdown: false,
      notifications: [],
  ...
  methods: {
    ...
    await axios
     .post("http://localhost:8000/api/seeResNotif", {
  ...
  mounted() {
    ...
    axios
      .post("http://localhost:8000/api/unseenResNotifNbr", {
        id: localStorage.getItem("id"),
      })
      .then((response) => {
        // Assign the notifications to the component's data property
